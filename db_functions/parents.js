...
const Parents = {
  create: async (user, id) => {
    const data = await CreateData("parents", user, "parent_id", id);
    return data;
  },
  update: async (user, id, learnerid) => {
    //Update the parent data
    const data = await UpdateData("parents", user, "parent_id", id);
    ...
    for (let idx of learnerid) {
      ...
      if (!learnerIsLinked) {
        await LinkLearnerData("parents_learners", {
          parent_id,
          learner_id: idx,
          year,
          session_id,
      ...
      await UpdateLearnerData("parents_learners", { branch_id }, insertid);
    }
