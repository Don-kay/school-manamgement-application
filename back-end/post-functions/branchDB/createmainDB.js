...
const FetchSinglemainDB = require("../branchDB/fetchSinglemainDB");
const { DataError } = require("../../error");

const CreatemainDB = async (table, updates, rowId1, insertId, rowId2, opt) => {
  ...
  // console.log(values);
  // Execute the query
  const create = await masterPool.query(query, values, (err, result) => {
    if (err) {
      throw err;
    } else {
  ...
  try {
    if (create[0].affectedRows > 0) {
      const fetchData = await FetchSinglemainDB(
        table,
        rowId1,
        insertId,
        rowId2,
